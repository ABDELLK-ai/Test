...
%% Chargement des paramètres de simulation du modèle non linéaire MNL
...
xlabel('t(s)')
ylabel('V_y (m/s)')
title('vitesse latérale modèle non linéaire')
grid on
...
title('vitesse de lacet modèle non linéaire')
grid on


figure(Name='accélération latérale du modèle non linéaire')
plot(t, gamma_t,'c')
xlabel('t(s)')
ylabel('Gamma_t (m/s²)')
